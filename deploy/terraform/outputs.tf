...
output "s3_bucket_name" {
  value       = aws_s3_bucket.persistent_data.id
...
output "ssh_connect_command" {
  value       = var.ssh_key_name != "" ? "ssh -i ~/.ssh/${var.ssh_key_name}.pem ubuntu@${aws_eip.main.public_ip}" : "SSH key not configured - use SSM Session Manager"
